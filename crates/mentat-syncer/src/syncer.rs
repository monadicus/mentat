    ...
    Help: 'static + Helper + Send + Sync + Clone,
{
    ...
    ) -> SyncerResult<Option<usize>> {
        // Always fetch network status to ensure end_index is not past tip
        let network_status = self
            .helper
            .network_status(context, &self.network)
            .map_err(|e| {
    ...
    fn process_blocks(
        ...
        while self.next_index <= end_index {
            let br = if let Some(br) = cache.remove(&self.next_index) {
                ...
                // in a reorg.
                if !matches!(reorg_start, Some(i) if i >= self.next_index) {
                    break;
                }
            ...
            if self.next_index < last_processed && reorg_start.is_none() {
                reorg_start = Some(last_processed)
            }
        }
    ...
    fn sequence_blocks(
        ...
        for result in fetched_blocks_receiver {
            ...
            self.recent_block_sizes.push_back(result.estimated_size());
    ...
    fn sync_range(&mut self, context: &Context<SyncerError>, end_index: usize) -> SyncerResult<()> {
        ...
        self.recent_block_sizes = [0; DEFAULT_TRAILING_WINDOW].into();
        self.last_adjustment = 0;
        ...
        *self.goal_concurrency.lock() = starting_concurrency;
        ...
        let fetcher_index = Arc::new(AtomicUsize::from(self.next_index));
    ...
    /// range is synced, all fetcher threads are shutdown. if `start_index` is
    /// none then the syncer will start from the genesis block. if `end_index`
    /// is none then the syncer will sync until it reaches tip or the provided
    /// context expires
    pub fn sync(
        &mut self,
        context: &Context<SyncerError>,
        ...
        loop {
            ...
            let range_end = if let Some(i) = range_end {
                i
            } else if matches!(end_index, Some(i) if self.next_index > i) {
                break;
            } else {
