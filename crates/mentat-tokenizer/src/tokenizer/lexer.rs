...
use crate::{errors::Result, TokenKind};

/// Type
pub trait Lexer: TokenKind {
    fn tokenize_single(input: &str) -> Result<(Self, usize)>
    where
        Self: Sized;
...
//         } else if seen_dot {
//             let n: f64 =
// LexerError::could_not_lex_decimal_number(number.parse(), number)?;
//             Ok((TokenKind::from(n), bytes_read))
//         } else if negated {
//             let n: isize =
// LexerError::could_not_lex_signed_number(number.parse(), number)?;            
// Ok((TokenKind::from(n), bytes_read))         } else {
//             let n: usize = LexerError::could_not_lex_number(number.parse(),
// number)?;             Ok((TokenKind::from(n), bytes_read))
//         }
//     }

//     fn tokenize_ident(input: &mut Peekable<impl Iterator<Item = char>>) ->
// Result<(Self, usize)> {         // identifiers can't start with a number
//         match input.peek() {
//             Some(ch) if ch.is_ascii_digit() => return
// LexerError::ident_started_with_a_number(),             None => return
// LexerError::unexpected_eof(),             _ => {}
//         }

//         let ident: String =
//             std::iter::from_fn(|| input.next_if(|c| c.is_ascii_alphanumeric()
// || c == &'_'))                 .collect();
...
//                     let comment = match
// input_str.as_bytes().iter().position(|c| *c == b'\n') {                      
// None => input_str,                         Some(idx) => &input_str[..idx +
// 1],                     };
//                     Ok((TokenKind::Comment(comment.to_owned()),
// comment.len()))                 } else {
//                     
...
//                 let string = match rest.as_bytes().iter().position(|c| *c ==
// b'"') {                     None => {
...
//         let (got, _bytes_read) =
// TokenKind::tokenize_single($src.into()).unwrap();         assert_eq!(got,
// should_be, "Input was {:?}", $src);     };
//     (@inner INT $src:expr, $should_be:expr) => {
//         let should_be = TokenKind::from($should_be);

//         let (got, _bytes_read) =
//             TokenKind::tokenize_number(&mut
// $src.chars().peekable()).unwrap();         assert_eq!(got, should_be, "Input
// was {:?}", $src);     };
//     (@inner IDENT $src:expr, $should_be:expr) => {
//         let should_be = TokenKind::from($should_be);

//         let (got, _bytes_read) = TokenKind::tokenize_ident(&mut
// $src.chars().peekable()).unwrap();         assert_eq!(got, should_be, "Input
// was {:?}", $src);     };
...
// lexer_test!(IDENT: tokenize_ident_containing_an_underscore, "Foo_bar" =>
// "Foo_bar"); lexer_test!(
...
// lexer_test!(INT: tokenize_a_longer_negative_integer, "-1234567890" =>
// -1234567890isize); lexer_test!(INT: tokenize_basic_decimal, "12.3" => 12.3);
// lexer_test!(INT: tokenize_string_with_multiple_decimal_points, "12.3.456" =>
// 12.3); lexer_test!(
...
// lexer_test!(SINGLE: central_tokenizer_comment, "// comment" =>
// TokenKind::Comment("// comment".into())); lexer_test!(SINGLE:
// central_tokenizer_ampersand, "&" => TokenKind::Ampersand); lexer_test!
// (SINGLE: central_tokenizer_asterisk, "*" => TokenKind::Asterisk); lexer_test!
// (SINGLE: central_tokenizer_colon, ":" => TokenKind::Colon); lexer_test!
// (SINGLE: central_tokenizer_dot, "." => TokenKind::Dot); lexer_test!(SINGLE:
// central_tokenizer_open_square, "[" => TokenKind::LeftBracket); lexer_test!
// (SINGLE: central_tokenizer_open_close_square, "[]" => TokenKind::ArrayType);
// lexer_test!(SINGLE: central_tokenizer_close_square, "]" =>
// TokenKind::RightBracket); lexer_test!(SINGLE: central_tokenizer_open_brace,
// "{" => TokenKind::LeftCurly); lexer_test!(SINGLE:
// central_tokenizer_open_close_brace, "{}" => TokenKind::DefaultObject);
// lexer_test!(SINGLE: central_tokenizer_close_brace, "}" =>
// TokenKind::RightCurly); lexer_test!(SINGLE: central_tokenizer_open_paren, "("
// => TokenKind::LeftParen); lexer_test!(SINGLE: central_tokenizer_close_paren,
// ")" => TokenKind::RightParen); lexer_test!(SINGLE: central_tokenizer_ident,
// "Foo_bar" => "Foo_bar"); lexer_test!(SINGLE: central_tokenizer_int_type,
// "Int" => TokenKind::IntType); lexer_test!(SINGLE: central_tokenizer_new_int,
// "NewInt" => TokenKind::NewInt); lexer_test!(SINGLE:
// central_tokenizer_interface, "interface" => TokenKind::Interface);
