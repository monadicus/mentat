...
// PartialBlockIdentifier},     requests::{AccountBalanceRequest,
// AccountCoinsRequest, MetadataRequest},     serde_json::json,
//     tokio,
// };
...
//     let get_first_network = || network::first_network_or_null(&client,
// main_opts.net_id());
...
//                 display!(
//                     client.block_transaction(&(network, block,
// transaction.clone().into()).into())                 );
//             } else {
//                 // find a specific block
//                 let block = main_opts
//                     .partial_block_id()
//                     .expect("Expected a partial block identifier (--hash,
// --index)");                 display!(client.block(&(network, block).into()));
//             }
//         }
...
//             // handle --transaction flag
//             if let Some(transaction) = &opts.transaction {
//                 display!(client.mempool_transaction(&(network,
// transaction.clone().into()).into()));             } else {
